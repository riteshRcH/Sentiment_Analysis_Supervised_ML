...
namespace CrawlNScrape
{
    class BlogMining
    {
        static List<CrawlNScrape.ReviewSite> links2Crawl = new List<CrawlNScrape.ReviewSite>();
        ...
        static void Main(string[] args)
        {
            ...
            {
                Console.WriteLine("Site: " + site.ReviewSiteURL.ToString() + "\n");
                Crawl(site);
            }

            Console.Write("\n\nBuilding Movie Names Menu....");
            buildMovieMenu();
            Console.WriteLine("Done!");

            Console.WriteLine("\n\nMenu: ");
            for(int i=0;i<finalMovieMenuList.Count;i++)
                Console.WriteLine((i+1) + " => " + finalMovieMenuList[i]);
            ...
            do
            {
                try
                {
                    Console.WriteLine("\n\nEnter your choice(s) -- comma seperated: ");
                    string[] inputs;
                    if (trainSys)
                    {
                        ...
                        for (int i = 0; i < finalMovieMenuList.Count; i++)
                        {
                            if (i < finalMovieMenuList.Count - 1)
                                trainingInput += (i + 1).ToString() + ",";
                            else
                                trainingInput += (i + 1).ToString();
                        }
                        Console.WriteLine(trainingInput);
                        inputs = trainingInput.Replace(" ", "").Trim().Split(',');
                    }
                    else
                        //inputs = "1, 5, 6, 9, 11, 13, 17, 18, 19, 20, 22, 23, 27, 30, 36, 37, 40, 50, 54, 55, 60, 61, 65, 73, 74, 75, 76, 78, 79, 84, 85, 87, 88, 91, 94, 97, 98, 100, 102, 113, 123, 124, 127, 128, 130, 131, 132, 134, 135".Replace(" ", "").Trim().Split(',');
                        inputs = Console.ReadLine().Replace(" ", "").Trim().Split(',');

                    foreach (string input in inputs)
                        moviesChosen.Add(finalMovieMenuList[Int32.Parse(input) -1]);

                    Console.WriteLine("You chose: ");
                    int k = 0;
                    foreach (string movie in moviesChosen)
                        Console.WriteLine(inputs[k++]+"  =>  "+movie);
                    break;
                }
                catch (Exception e)
                {
                    Console.WriteLine("Invalid input" + e.Message + "\n"+e.StackTrace+"please try again");
                    moviesChosen.Clear();
                    continue;
                }
            } while (true);
            //while (movieChoice < 0 || movieChoice > finalMovieMenuList.Count);
            ...
            foreach (string movie in moviesChosen)
            {
                ...
                else if (movie.Equals("heroine"))
                {
                    getLinksForSpecialCases("heroine movie review taran rajeev hindi komal anupama");
                    getLinksForSpecialCases("heroine music songs");
                }
                else if (movie.Equals("oh my god"))
                {
                    getLinksForSpecialCases("omg oh my god");
                    getLinksForSpecialCases("omg oh my god anupama komal");
                }
                else if (movie.Equals("resident evil retribution"))
                    getLinksForSpecialCases("resident evil 5 retribution");
                else if (movie.Equals("avengers"))
                    getLinksForSpecialCases("the avengers");
            ...
            //FileStream fout = new FileStream("C:\\Users\\Ritesh\\Desktop\\hehe.txt", FileMode.OpenOrCreate, FileAccess.ReadWrite);
            //foreach (String s in links2Scrape4UserSelectedMovies)
                //fout.Write(new ASCIIEncoding().GetBytes((s + "\n").ToCharArray()), 0, new ASCIIEncoding().GetBytes((s + "\n").ToCharArray()).Length);
            //fout.Close();
            ...
            for (int i = 0; i < m.Length; i++)
                Console.WriteLine(m[i].movieName + "\t\t" + m[i].rating);

            if (trainSys)
            {
                SqlConnection sqlConnection = new SqlConnection("server=localhost\\SqlExpress; DataBase=WordDBForBlogMiner; integrated security=true");
                sqlConnection.Open();
                SqlCommand delQuery = new SqlCommand("delete from trainingDataTable", sqlConnection);
                delQuery.ExecuteNonQuery();
                SqlCommand insertQuery;
                for (int i = 0; i < m.Length; i++)
                {
                    if (m[i].posScore > 0 && m[i].posWordCnt > 0 && m[i].negScore > 0 && m[i].negWordCnt > 0 && m[i].rating > 1)
                    {
                        insertQuery = new SqlCommand("insert into trainingDataTable values(" + (i + 1) + " ,'" + m[i].movieName + "', " + m[i].posScore + ", " + m[i].posWordCnt + ", " + m[i].negScore + ", " + m[i].negWordCnt + ", " + m[i].rating + ")", sqlConnection);
                        insertQuery.ExecuteNonQuery();
                        Console.WriteLine("Successfully inserted movie " + (i + 1) + " training data");
                    }
                }
                sqlConnection.Close();
            }
        ...
        private static double getOfficialMovieRating(String[] links2getOfficialRating)
        {
            try
            {
                foreach (String link in links2getOfficialRating)
                {
                    if (link.Contains("wogma.com"))
                    {
                        ...
                        {
                            officialScore += Double.Parse(s);
                            cnt++;
                        }

                        //Console.Write("Official Wogma Rating: " + ((officialScore*5)/(cnt*5)));

                        return ((officialScore * 5) / (cnt * 5));
                    }
                    else if (link.Contains("koimoi"))
                    {
                        ...
                        //Console.WriteLine("Official Koimoi Rating: " + Double.Parse(extractedContent));
                    ...
                    else if (link.Contains("bollymoviereviewz.blogspot.in"))
                    {
                        ...
                        //Console.WriteLine("Official blogspot wale ka rating: "+double.Parse(extractedContent));
                    ...
                    else if (link.Contains("hindustantimes.com"))
                    {
                        ...
                        //Console.WriteLine("Official HT Rating: " + officialScore);
                    ...
                    else if (link.Contains("themoviereviewblog.wordpress.com"))
                    {
                        ...
                        //Console.WriteLine("Official wordpress wale ka rating: " + officialRating);
        ...
        static void Crawl(ReviewSite site)
        {
            try
            {
                Console.Write("\tCrawling Started....\t");
                ...
                Console.WriteLine("Done!");
                Console.Write("\tGathering Movie Names....");
        ...
        static void buildMovieMenu()
        {
            ...
            */
            ...
            for (int i = 0; i < movieMenuList.Count; i++)
            {
                ...
                else if (movieMenuList[i].Contains("heroine music songs") || movieMenuList[i].Contains("heroine movie review taran rajeev hindi komal anupama"))
                    movieMenuList[i] = "heroine";
                else if (movieMenuList[i].StartsWith("omg oh my god"))
                    movieMenuList[i] = "oh my god";
                else if (movieMenuList[i].Contains("resident evil 5 retribution"))
                    movieMenuList[i] = "resident evil retribution";
                else if (movieMenuList[i].Contains("the avengers"))
                    movieMenuList[i] = "avengers";
            }
            ...
            movieMenuList.Remove("overtime");
            movieMenuList.Remove("krishna aur kans");
            movieMenuList.Remove("the lost tape");
            movieMenuList.Remove("1920 evil returns");
            movieMenuList.Sort();
